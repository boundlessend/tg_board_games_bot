...
Telegram-бот с помощниками для настольных игр. Помощники: «Опасные слова» (слова, проклятья, боссы), «Крокодил» и «Алиас» (слова для объяснения). Контент выдаётся без повторов для каждого Telegram ID, история хранится в SQLite. Есть инлайн-режим и админ-меню со статистикой и добавлением контента.
...
`tests/smoke.py` - smoke-проверка без сети и токена: загрузка и валидация контента, выдача без повторов, авто-цикл, пользовательский контент, аналитика и регистрация роутеров.
